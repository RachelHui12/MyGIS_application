...
namespace MyGIS_xsp
{
    ...
    {
        public BufferForm(AxMapControl hook)
        {
            InitializeComponent();
            this.mHookHelper.Hook = hook.Object;
            axmap = hook;
        }
        //flag表示是直接在图层里的缓冲区，还是从文件中构建的缓冲区
        bool flag = false;
        //接收MapControl中的数据
        private IHookHelper mHookHelper = new HookHelperClass();
        AxMapControl axmap = null;
        //缓冲区文件输出路径
        public string strOutputPath;
        public string filePath="", fileName="",filefullpath="";
        ...
        private void btnBuffer_Click(object sender, EventArgs e)
        {
            ...
            if (fileName=="")
            {
                ...
                IFeatureLayer pFeatureLayer = GetFeatureLayer(fileName.Substring(0,index));
                //获取一个geoprocessor的实例
                Geoprocessor gp = new Geoprocessor();
                //OverwriteOutput为真时，输出图层会覆盖当前文件夹下的同名图层
                gp.OverwriteOutput = true;
        ...
        private void btninput_Click(object sender, EventArgs e)
        {
            ...
            {
                cboLayers.Text = openFileDialog.FileName;
                txtOutputPath.Text = cboLayers.Text.Substring(0, cboLayers.Text.Length - 4) + "_buffer.shp";
                int Index = cboLayers.Text.LastIndexOf("\\");
                filePath = cboLayers.Text.Substring(0, Index);
                fileName = cboLayers.Text.Substring(Index + 1);
                filefullpath = cboLayers.Text;
                axmap.AddShapeFile(filePath,fileName);
            }
            else { 
